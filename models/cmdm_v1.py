...
class CMDM(nn.Module):
    ...
    def __init__(self, cfg: DictConfig, *args, **kwargs):
        ...
        if self.arch == 'trans_wkv_v7':
            self.self_attn_layers = nn.ModuleList()
            self.kv_mappling_layers = nn.ModuleList()
            self.cross_attn_layers = nn.ModuleList()

            # [新增] 为 RWKV 模块创建一个配置对象
            rwkv_args = Namespace(
                n_embd=self.latent_dim,
                n_layer=sum(self.num_layers), # RWKV需要总层数来进行初始化
                dim_att=self.latent_dim, # 通常与 n_embd 相同
            ...
            for i, n in enumerate(self.num_layers):
                ...
                # [修改] 将 TransformerEncoder 替换为我们的双向 RWKV 模块
                # 我们使用 nn.Sequential 将 n 个 RWKV Block 堆叠起来
                self.self_attn_layers.append(
                    nn.Sequential(
                        *[BidirectionalRWKVBlock(rwkv_args, global_layer_id + j) for j in range(n)]
                    )
                )
                global_layer_id += n
